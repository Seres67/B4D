...
public class JPanel_Programme extends JPanel {
    ...
    public JPanel_Programme(B4D b4d) {
        ...
        button_Start.addActionListener(arg0 -> {
            if (table.getSelectedRow() != -1) {
                Program program = b4d.getPrograms().get(table.getSelectedRow());
                ProgramOptions programOptions = new ProgramOptions(Integer.parseInt(textField_Turns.getText()), Long.parseLong(textField_Delay.getText()), checkBox_HDV.isSelected(), checkBox_Bank.isSelected(), checkBox_Stop.isSelected());
    ...
    private void checkStartButton(B4D b4d) {
        button_Start.setEnabled(b4d.getConfiguration().isComplet() && !b4d.getTeam().isEmpty() && table.getSelectedRow() != -1);
    }
}